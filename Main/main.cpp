...
//    pixi::files::file mwds("../../Kernel/data/MalwareDataSet.data");
    ...
    auto www = [&](const dword min_it, const dword max_it) {
        ...
        while (c--)
        {
            ...
            if (err < minerr) {
                minerr = err;
                std::cout << "\nNNID" << std::this_thread::get_id() << ": " << minerr << std::endl
                          << "Kohonen layer: " << lw << " Iterations: " << li << std::endl;
                mtx.lock();
                d1 = det1;
                mtx.unlock();
            }
            std::cout << "NNID" << std::this_thread::get_id() << "Progress: " << c << std::endl;
        }
        std::cout << "NNID" << std::this_thread::get_id() << ": work done!" << std::endl;
    };

    std::thread t1(www, 100, 1000), t2(www, 100, 1000);
